...
public:
	...
	ImGUIZMO(VulkanExampleBase *example) : example(example)
	{
	...
	void newFrame(VulkanExampleBase *example, bool updateFrameGraph,ImVec2 viewport_position,ImVec2 viewport_size,std::set<Entity>& entities )
	{
		...
		for (auto& entity : entities)
		{
			...
			for (auto& node : renderable.model.nodes)
            {
                ...
                ImGuizmo::Manipulate(glm::value_ptr(renderable.updateViewMatrix(example)), glm::value_ptr(cam.projection), ImGuizmo::TRANSLATE,  ImGuizmo::LOCAL, glm::value_ptr(nodeMatrix), NULL, NULL);
            }
